...
CITY_TO_STATE = {
    ...
    'minneapolis': 'MN', 'st paul': 'MN', 'rochester': 'MN', 'bloomington': 'MN', 'shakopee': 'MN',
...
class UnifiedJobAggregator:
    def __init__(self):
        ...
        print(f"Loaded: {len(self.existing_jobs)} jobs, {len(self.existing_urls)} URLs, {len(self.existing_job_ids)} IDs")
    ...
    def extract_job_age_days(self, soup):
        ...
        try:
            ...
            match = re.search(r'[Pp]osted:?\s*(\d{1,2})[/\-](\d{1,2})[/\-](\d{2,4})', page_text)
            if match:
    ...
    def parse_ziprecruiter_email_enhanced(self, soup, url):
        ...
        try:
            ...
            job_cards = soup.find_all(['div', 'table', 'tr'], class_=re.compile(r'job|listing', re.I))
                ...
                if url_found:
                            ...
                            if len(parts) >= 2:
                                ...
                                if len(parts) >= 3:
                                    ...
                                    if 'remote' in work_type:
                                        ...
                                        remote = 'Hybrid'
                                    elif 'onsite' in work_type or 'in-person' in work_type:
                                        remote = 'On Site'
                    
                    # Fallback: look for city, state pattern
                    if location == 'Unknown':
                        loc_match = re.search(r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?),\s*([A-Z]{2})\b', card_text)
                        if loc_match:
                    ...
                    if title and title != 'Unknown':
                        return {
                            ...
                            'url': url,
                            'job_id': 'N/A',
                            'remote': remote if remote != 'Unknown' else self.infer_remote_from_text(location),
            ...
            for i in range(max(0, url_index - 10), url_index):
                line = lines[i]
                if 10 < len(line) < 150:
                    if any(kw in line.lower() for kw in ['intern', 'engineer', 'developer', 'software', 'swe']):
                        if '•' not in line and 'View' not in line and 'Apply' not in line and '$' not in line:
                            title = line
                            break
        ...
        try:
            ...
            for h2 in h2_tags:
                ...
                if next_elem:
                    ...
                    for line in lines:
                        if any(skip in line.lower() for skip in ['more details', 'view details', 'new', 'top match']):
                            continue
                        ...
                        if ' - ' in line and len(line) < 100:
                            ...
                            if len(parts) >= 2:
                                ...
                                location = re.sub(r'\s*(Hybrid|Remote|In-person)$', '', location_raw, flags=re.I).strip()
                                break
        ...
        if 'jobright.ai/jobs/info/' in final_url.lower():
            ...
            if self.is_external_job_board(final_url):
                ...
                if company and title_raw:
                    location = self.extract_location_enhanced(soup, final_url)  # NEW: Enhanced extraction
                    job_id = self.extract_job_id_from_page(soup, final_url)
                    remote_status = self.extract_remote_status_enhanced(soup, location, final_url)  # NEW
            ...
            title_no_location = self.remove_location_from_title(title_raw)
            title_final = self.clean_title_aggressive(title_no_location)  # NEW: Aggressive cleaning
            ...
            job_id = self.extract_job_id_from_page(soup, final_url)
            location_extracted = self.extract_location_enhanced(soup, final_url)  # NEW: Enhanced extraction
            remote = self.extract_remote_status_enhanced(soup, location_extracted, final_url)  # NEW
            sponsorship = self.check_sponsorship_status(soup)
            ...
            location_intl_check = self.check_if_international_location_strict(location_extracted, soup)
    ...
    def check_if_international_location_strict(self, location, soup):
        ...
        canadian_cities_definite = [
            ...
            'hamilton', 'kitchener', 'waterloo', 'halifax', 'victoria'
        ]
        ...
        if re.search(r'\bcanada\b', location_lower):
            ...
            us_city_count = sum(1 for city in CITY_TO_STATE.keys() if city in location_lower)
    ...
    def detect_country_from_page_content_strict(self, soup):
        ...
        try:
            ...
            us_city_mentions = sum(1 for city in list(CITY_TO_STATE.keys())[:20] if city in page_text.lower())
    ...
    def clean_title_aggressive(self, title):
        ...
        title = re.sub(r'\s*(Summer|Fall|Winter|Spring)\s*20\d{2}\s*', ' ', title, flags=re.I)
        ...
        title = re.sub(r'\s*[-–]\s*(Full[- ]time|Part[- ]time)\s*$', '', title, flags=re.I)
    ...
    def is_valid_us_location(self, location):
        ...
        has_us_city = any(city in location_lower for city in list(CITY_TO_STATE.keys())[:50])
        
        # Check for Canadian indicators
        has_canada = 'canada' in location_lower or any(prov in location for prov in CANADA_PROVINCES)
    ...
    def extract_location_from_labels_enhanced(self, soup):
        ...
        try:
            ...
            labels = ['location:', 'office location:', 'work location:', 'job location:']
            ...
            for label in labels:
                ...
                for tag in soup.find_all(['span', 'div', 'p'], string=re.compile(label, re.I)):
                    parent = tag.parent
                    if parent:
        ...
        try:
            ...
            for city, state in matches:
                if state.upper() in US_STATES.values():
                    ...
                    context = page_text[max(0, page_text.find(full_match)-50):page_text.find(full_match)+100]
                    
                    # Skip if it looks like it's in a weird context
                    if any(skip in context.lower() for skip in ['copyright', 'reserved', 'patent', 'trademark']):
                        continue
    ...
    def auto_resize_columns_with_status_dynamic(self, sheet, url_column_index, total_columns):
        ...
        try:
            ...
            fixed_widths.append({
                "updateDimensionProperties": {
                    ...
                    "properties": {"pixelSize": max(max_status_length, 100)},  # Minimum 100 pixels
                    "fields": "pixelSize"
                }
            })
    ...
    def auto_resize_all_columns_except_url(self, sheet, url_column_index, total_columns):
        ...
        self.auto_resize_columns_with_status_dynamic(sheet, url_column_index, total_columns)
    ...
    def process_job_url(self, url, company, title):
        try:
            ...
            job_id = self.extract_job_id_from_page(soup, final_url)
            location = self.extract_location_enhanced(soup, final_url)  # NEW
            remote = self.extract_remote_status_enhanced(soup, location, final_url)  # NEW
            sponsorship = self.check_sponsorship_status(soup)
    ...
    def scrape_simplify_github(self):
        ...
        try:
            ...
            for table in tables:
                ...
                for row in rows[1:]:
                    ...
                    if discard_reason != "Filtered":
                        country = self.detect_country_simple(location) if discard_reason.startswith("Location") else self.format_location_clean(location)
                    ...
                    if result['status'] == 'rejected':
                        ...
                        location_check = self.check_if_international_location_strict(result.get('location'), None)
                        ...
                        if location_check:
                            ...
                            self.valid_jobs.append({
                                ...
                                'job_type': self.determine_job_type(title),
                                'location': self.format_location_clean(result.get('location')),  # NEW
                                'remote': result.get('remote'),
                                'entry_date': self.format_date(),
                                'url': result['final_url'],
                                'source': 'GitHub',
            ...
            print(f"GitHub: {len([j for j in self.valid_jobs if j['source'] == 'GitHub'])} valid\n")
